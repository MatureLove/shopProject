...
export default {
  ...
  methods: {
    ...
    //加入购物车
    async addShopCart() {
      //发请求，然后台将数据存到数据库中，需要带商品的id，和购买数量
      try {
        //在这里我们可以利用await来接受调用addShopCart返回的promise成功的值，但他必须在async中使用，所以将这个函数写为async函数
        //但是如果失败，我们需要用trycatch来捕获
        await this.$store.dispatch('detail/addShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuNum })
        //发送请求成功之后进行路由跳转，需要带上购买产品的信息和数量
        //简单的数据我们通过路由传参
        //复杂的我们利用回话存储技术，sessionStorage
        //因为skuInfo是个对象，回话里面没办法直接存，需要先转成json格式的字符串JSON.stringify
        ...
        //当await右边是一个失败的promise对象时，error接受的时失败promise的结果值
        alert(error.message)
      }
    }
  }
