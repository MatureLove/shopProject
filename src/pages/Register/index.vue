<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>确认密码:</label>
        <input placeholder="请确认你的密码" v-model="rpassword" name="rpassword" v-validate="{ required: true, is: password }"
          :class="{ invalid: errors.has('rpassword') }" />
        <span class="error-msg">{{ errors.first("rpassword") }}</span>
      </div>
      <div class="controls">
        <input type="checkbox" v-model="checked" name="agree" v-validate="{ required: true, agree: true }"
          :class="{ invalid: errors.has('agree') }" />
        <span>同意协议并注册《尚品汇用户协议》</span>
        <span class="error-msg">{{ errors.first("agree") }}</span>
      </div>
      <div class="btn">
        <button @click="submitInfo">完成注册</button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async getCode() {
      this.phone && await this.$store.dispatch('user/getPhoneCode', this.phone)
